...
import { CheckCircle, FileText, Film, Timer, Play, FileQuestion, ExternalLink } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 grid grid-cols-1 lg:grid-cols-12 gap-6">
        <aside className="lg:col-span-4 xl:col-span-3 border border-slate-200 rounded-2xl overflow-hidden">
          <div className="p-4 border-b border-slate-200">
            <div className="flex items-center justify-between">
              <div className="font-semibold">Course Content</div>
              <div className="text-sm text-slate-600 inline-flex items-center gap-1"><Timer className="w-4 h-4" /> {Math.ceil(totalDuration/60)}m</div>
            </div>
            <div className="mt-2 h-2 bg-slate-100 rounded-full overflow-hidden">
              <div className="h-full bg-emerald-600" style={{ width: `${progressPercent}%` }} />
            </div>
            <div className="mt-1 text-xs text-slate-600">{progressPercent}% completed</div>
          </div>
