      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2">
            ...
            {course.summary && <p className="mt-2 text-slate-800 text-lg leading-relaxed">{course.summary}</p>}
            {course.description && (
              <div className="mt-6 prose max-w-none prose-p:text-slate-900 prose-li:text-slate-900">
                <p className="whitespace-pre-line text-slate-900 leading-relaxed">{course.description}</p>
              </div>
            )}
            ...
            <div className="mt-10">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-4">
                <div className="p-4 rounded-xl border border-slate-200 bg-white flex items-center gap-3 shadow-sm">
                  ...
                  <span className="text-slate-800 text-sm font-medium">Lifetime access</span>
                </div>
                <div className="p-4 rounded-xl border border-slate-200 bg-white flex items-center gap-3 shadow-sm">
                  <Clock className="w-5 h-5 text-emerald-600" />
                  <span className="text-slate-800 text-sm font-medium">Learn at your pace</span>
                </div>
                <div className="p-4 rounded-xl border border-slate-200 bg-white flex items-center gap-3 shadow-sm">
                  <Star className="w-5 h-5 text-amber-500" />
                  <span className="text-slate-800 text-sm font-medium">Certificate on completion</span>
                </div>
                <div className="p-4 rounded-xl border border-slate-200 bg-white flex items-center gap-3 shadow-sm">
                  <Play className="w-5 h-5 text-emerald-600" />
                  <span className="text-slate-800 text-sm font-medium">Interactive content</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div>
            <div className="sticky top-6 p-6 rounded-2xl border border-slate-200 shadow-sm bg-white">
              <div className="flex items-center justify-between mb-4">
                <span className="text-slate-700 font-medium">Price</span>
                <span className="text-2xl font-bold text-slate-900">{priceText}</span>
              </div>
              <button
                onClick={handleEnroll}
                disabled={isEnrolling}
                className="w-full inline-flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-700 disabled:bg-emerald-400 text-white font-semibold py-3 rounded-xl transition-colors"
              >
                {isPaid ? <ShoppingCart className="w-5 h-5" /> : <Play className="w-5 h-5" />}
                {isPaid ? (isEnrolling ? 'Processing...' : 'Buy & Enroll') : (isEnrolling ? 'Enrolling...' : 'Enroll Now')}
              </button>
              {isPaid && (
